...
async function processOneJob(jobId: string, markdown: string, pool: Awaited<ReturnType<typeof getPool>>) {
  ensurePdfStorageDir();
  const docLang = process.env.DOCUMENT_LOCALE ?? "en";
  const html = await renderMarkdownToHtmlDocument(markdown, docLang);
...
async function main(): Promise<void> {
  ...
  for (;;) {
    try {
      ...
      try {
        await processOneJob(claimed.id, claimed.markdown, pool);
      } catch (error: unknown) {
